...
with open("all.txt", 'w') as file:
    q = '''SELECT name,SGPA FROM SE_COMPUTER_2024_JUNE WHERE div NOT NULL and SGPA!='--' ORDER by SGPA DESC LIMIT 5;'''
    cursor.execute(q)
    ...
    for key,values in dic.items():
        if values[1] ==True:
            query0 = f"SELECT count(name) FROM SE_COMPUTER_2024_JUNE WHERE _{key}_end!='AB' and div NOT NULL;"
            query1 = f"SELECT count(name) FROM SE_COMPUTER_2024_JUNE WHERE _{key}_end='AB' and div NOT NULL;"
            query2 = f"SELECT count(name) FROM SE_COMPUTER_2024_JUNE WHERE _{key}_tot>=40 and div NOT NULL;"
            query3 = f"SELECT count(name) FROM SE_COMPUTER_2024_JUNE WHERE _{key}_tot<40 and div NOT NULL;"
    ...
    for key,values in dic.items():
        if values[1] ==True:
            query1 = f'SELECT name,_{key}_tot FROM SE_COMPUTER_2024_JUNE WHERE _{key}_end!="AB" and div NOT NULL ORDER by _{key}_tot DESC LIMIT 1;'
    ...
    query1 = '''SELECT count(name) FROM SE_COMPUTER_2024_JUNE WHERE div NOT NULL and SGPA!='--' and SGPA>=7.75;'''
    query2 = '''SELECT count(name) FROM SE_COMPUTER_2024_JUNE WHERE div NOT NULL and SGPA!='--' and SGPA<7.75 and SGPA>=6.75;'''
    query3 = '''SELECT count(name) FROM SE_COMPUTER_2024_JUNE WHERE div NOT NULL and SGPA!='--' and SGPA<6.75;'''
    query4 = '''SELECT count(name) FROM SE_COMPUTER_2024_JUNE WHERE div NOT NULL and SGPA='--' ;'''
